...
pc1.ontrack =  async(event) =>
{
    ...
    if(recievedStream.getVideoTracks()[0].kind === "peerScreen")
        {
            if(remoteVideo.classList.contains('mirror') ===true)    
                remoteVideo.classList.remove('mirror');    
            
        }
    else if(recievedStream.getVideoTracks()[0].kind === "peerVideo"){
        if(!(remoteVideo.classList.contains('mirror') ===true))    
        ...
        if(desc)
        {
            ...
            if(desc.type === 'offer')
            {
             ...
             /*  await  navigator.mediaDevices.getUserMedia({video:true}).then((mediastream)=>
                ...
                {
                  rtc_sender =   pc1.addTrack(track,localStream);
...
micButton.onclick = async()=>
{
    ...
    if(isMute === true)
    {
        ...
        dataChannel.send({type:"MEDIA_SETTINGS",isMute:true});
    }
    else
    {   
        ...
        dataChannel.send({type:"MEDIA_SETTINGS",isMute:false});
    }
